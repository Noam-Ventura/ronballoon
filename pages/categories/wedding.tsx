  ...
  return (
    <Layout>
      ...
      <section className="container py-12 grid md:grid-cols-2 gap-10 items-center">
        ...
        <Reveal>
          <div>
            <h1 className="mb-4">חתונות</h1>
            <p className="text-gray-600">אוירה רומנטית ואלגנטית בעיצובים מרשימים ומוקפדים.</p>
          </div>
        </Reveal>
      </section>
          ...
          {images.map((src) => (
            <Reveal key={src}>
              <div className="aspect-square overflow-hidden rounded-xl border border-gray-100">
                <Image src={src} alt="חתונות - עיצוב בלונים" width={600} height={600} className="w-full h-full object-cover" />
              </div>
            </Reveal>
          ))}
        </div>
